"""
This module is copied over from scikit-learn with some tweaks to 
prevent centering datasets.
...
def _center_scale_xy(X, Y, scale=True, demean=True):
...
class _PLS(
    ...
    def decompose_coef(self, k=None):
        # decomposition of coef into orthogonal x rotations and 
        # their corresponding y loadings
    ...
    def kth_rotations(self, k=None):
        if k is None:
            return self.x_rotations_, self.y_rotations_
        x_rotations_ = np.dot(
            self.x_weights_[:, :k],
            pinv2(np.dot(self.x_loadings_[:, :k].T, self.x_weights_[:, :k]), check_finite=False),
        )
        y_rotations_ = np.dot(
            self.y_weights_[:, :k],
            pinv2(np.dot(self.y_loadings_[:, :k].T, self.y_weights_[:, :k]), check_finite=False),
        )
        return x_rotations_, y_rotations_
    
    # --- #

    def transform(self, X, Y=None, k=None, copy=True):
    ...
    def predict(self, X, copy=True, k=None):
    ...
    def fit_transform(self, X, y=None, k=None):
...
class PLSRegression(_PLS):
    ...
    def __init__(
        self, n_components=2, *, scale=True, demean=True, max_iter=500, tol=1e-06, copy=True
    ):
        super().__init__(
...
class PLSSVD(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    def __init__(self, n_components=2, *, scale=True, demean=True, copy=True):
        self.n_components = n_components
        self.scale = scale
        self.demean = demean
        self.copy = copy
